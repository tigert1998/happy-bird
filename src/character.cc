...
void CharacterImpl::Move(bool forward, float step){
	...
	btVector3 forwardDir = trans.getBasis()[1];
	step = 10;
	dynamic_cast<btRigidBody*>(object_)->applyForce(forwardDir * step * (forward?1:-1), btVector3(0,0,0));
	dynamic_cast<btRigidBody*>(object_)->setLinearVelocity(forwardDir * step * (forward?1:-1));
	object_->setInterpolationLinearVelocity(forwardDir * step * (forward?1:-1));
}
...
void CharacterImpl::Jump(float step){
	step = 10;
	dynamic_cast<btRigidBody*>(object_)->clearForces();
	dynamic_cast<btRigidBody*>(object_)->setLinearVelocity(btVector3(0,1,0) * step);
	object_->setInterpolationLinearVelocity(btVector3(0,1,0) * step);
}
